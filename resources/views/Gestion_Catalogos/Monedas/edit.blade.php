    ...
    <div class="col-xl">
        <div class="card mb-4">
            <div class="card-header d-flex justify-content-between align-items-center">
                <h5 class="mb-0">Actualizar Monedas</h5>
                <small class="text-muted float-end">Campos requeridos *</small>
            </div>
            <div class="card-body"> 
                <form action="{{ route('monedas.update', ['monedas' => $moneda->id]) }}" method="POST" enctype="multipart/form-data">
                    @csrf
                    @method('PUT')
                    <div class="row">
                        ...
                        <div class="col-md-6">
                            <div class="form-group">
                                ...
                                <input type="text" id="nombre" name="nombre" placeholder="Escribe tu nombre"
                                    class="form-control @error('nombre') is-invalid @enderror" value="{{ old('nombre',$moneda->nombre) }}">
                                @error('nombre')
                        ...
                        <div class="col-md-6">
                            <div class="form-group">
                                ...
                                <select id="estado" name="estado" class="form-select @error('estado') is-invalid @enderror">
                                    ...
                                    <option value="1" {{ old('estado',$moneda->estado) == '1' ? 'selected' : '' }}>Activo</option>
                                    <option value="0" {{ old('estado',$moneda->estado) == '0' ? 'selected' : '' }}>Inactivo</option>
                                </select>
                                @error('estado')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                        ...
                        <div class="col-md-12">
                            <div class="form-group">
                                ...
                                <textarea id="descripcion" name="descripcion" rows="3"
                                    class="form-control @error('descripcion') is-invalid @enderror">{{ old('descripcion',$moneda->descripcion) }}</textarea>
                                @error('descripcion')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                            </div>
