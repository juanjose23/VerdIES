...
document.addEventListener('DOMContentLoaded', function () {
    ...
    // Detecta el tema guardado en localStorage o establece "light" como predeterminado
    const savedTheme = localStorage.getItem('templateCustomizer-vertical-menu-template--Style') || 'light';
    applyTheme(savedTheme);

    // Al hacer clic, alterna entre "light" y "dark" y recarga la página
    themeToggle.addEventListener('click', function () {
        const newTheme = (localStorage.getItem('templateCustomizer-vertical-menu-template--Style') === 'light') ? 'dark' : 'light';
        localStorage.setItem('templateCustomizer-vertical-menu-template--Style', newTheme);
        location.reload(); // Recarga la página para aplicar el nuevo tema
    });

    // Función para aplicar el tema, ícono y texto
    function applyTheme(theme) {
        document.documentElement.setAttribute('data-theme', theme);

        if (theme === 'dark') {
            themeIcon.className = 'bx bx-moon bx-md';
            themeText.textContent = 'Modo: Oscuro';
        } else {
            themeIcon.className = 'bx bx-sun bx-md';
