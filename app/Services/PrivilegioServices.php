...
class PrivilegioServices
{
    protected $privilegioModel;
    public function __construct(Privilegios $privilegiosmodel)
    {
        $this->privilegioModel = $privilegiosmodel;
    }

    public function AsignarPrivilegio($rolId, $submodulos)
    {
        foreach ($submodulos as $submoduloIds) {
            foreach ($submoduloIds as $id_submodulo) {
                $privilegio = new $this->privilegioModel();
                $privilegio->roles_id = $rolId;
                $privilegio->submodulos_id = $id_submodulo;
                $privilegio->estado = 1;
                $privilegio->save();
            }
        }
    ...
    public function EliminarPrivilegio($privilegioId)
    {
        $privilegio = $this->privilegioModel::findOrFail($privilegioId);
        $privilegio->delete();
